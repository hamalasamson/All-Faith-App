...
export default function Login() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    toast({
      title: "Demo Mode",
      description: "Firebase not connected yet. Your app is ready for Firebase credentials.",
    });
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#0a1628] to-[#1e3a8a] px-4 py-10">
      ...
      >
        ...
        <div className="glass-card rounded-3xl p-6 space-y-4">
          ...
          <button
            onClick={() => toast({ title: "Demo Mode", description: "Google Sign-In requires Firebase to be connected." })}
            className="w-full flex items-center justify-center gap-3 py-3 rounded-xl bg-white/10 border border-white/20 text-white text-sm font-medium hover:bg-white/15 transition-all"
        ...
        <Link href="/">
          <p className="text-center text-white/40 text-xs mt-6 hover:text-white/60 transition-colors">
            Back to Home
          </p>
        </Link>
      </motion.div>
