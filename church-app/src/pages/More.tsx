...
import { Pin, Calendar, Bell, MapPin, CheckCircle, Facebook, Youtube } from "lucide-react";
...
function PrayerTab() {
  ...
  type PrayerEntry = { name: string; request: string; date: string };
  const [history, setHistory] = useState<PrayerEntry[]>(() => {
    try { return JSON.parse(localStorage.getItem("prayerRequests") || "[]"); } catch { return []; }
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const entry = { name: name.trim(), request: request.trim(), date: new Date().toLocaleDateString() };
  ...
  return (
    <div className="space-y-4">
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            >
              ...
              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-primary text-white font-semibold text-sm glow-blue hover:opacity-90 transition-all"
      ...
      {history.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2">Your Requests</h3>
          <div className="space-y-2">
            {history.slice(0, 3).map((entry, i) => (
              <div key={i} className="rounded-xl p-3 bg-card border border-border">
                <div className="flex justify-between text-[11px] mb-1">
                  <span className="font-semibold text-foreground">{entry.name}</span>
                  <span className="text-muted-foreground">{entry.date}</span>
                </div>
                <p className="text-xs text-muted-foreground line-clamp-2">{entry.request}</p>
              </div>
            ))}
          </div>
        </div>
